...
# openssl rsa -in FAprivkey.pem -pubout -out FApubkey.pem

# openssl rsa -in private_key.pem -noout -text



n=  "00d47b12733c93e802f6598beb3466" +\
...
d=    "37ed5503e3f40263953828adf612e5" +\
...
# 1. Calcola la lunghezza in byte del modulo (n)
lunghezza = (n_decimale.bit_length() + 7) // 8

# 2. Converte l'intero decifrato in bytes
msg_bytes = d_decimale.to_bytes(lunghezza, "big")

# 3. Rimuove eventuali 0x00 iniziali (solo se usi raw RSA senza padding)
msg_bytes = msg_bytes.lstrip(b'\x00')

# 4. Decodifica in UTF-8
